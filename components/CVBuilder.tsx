...
import EmailCollectionModal from './EmailCollectionModal'; // 🛑 AJOUT 1/5 : Import de la nouvelle modale
...
  const [isPrintModalOpen, setPrintModalOpen] = useState<boolean>(false);
  const [isEmailModalOpen, setIsEmailModalOpen] = useState<boolean>(false); // 🛑 AJOUT 2/5 : Nouvel état pour la modale Email
...
  }, []);
  ...
  // 🛑 AJOUT 3/5 : L'action de téléchargement réelle (appelée après la collecte d'email)
  const runActualPrint = useCallback(() => {
    setPrintModalOpen(false); // S'assurer que le mode aperçu est désactivé
    window.print(); // L'action qui lance le téléchargement (ou l'impression)
    setIsEmailModalOpen(false); // S'assurer que la modale de collecte est fermée
  }, []);
  
  // 🛑 MODIFICATION 4/5 : handlePrint ouvre maintenant la modale d'email
  const handlePrint = () => {
...
                  </button>
                  <button
                    onClick={handlePrint} // Appel la nouvelle logique (ouvre la modale)
...
        </PrintPreviewModal>
        ...
        {/* 🛑 AJOUT 5/5 : Ajout du composant modale de collecte d'email */}
        <EmailCollectionModal
            isOpen={isEmailModalOpen}
            onClose={() => setIsEmailModalOpen(false)}
            onSuccess={runActualPrint} // Appel la fonction de téléchargement après la soumission réussie
        />
